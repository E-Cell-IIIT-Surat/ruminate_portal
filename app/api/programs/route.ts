...
export async function POST(request: Request) {
  try {
    ...
    const program = await db.$transaction(
      async (tx) => {
        await reserveProgramSlug(tx, input.slug, actor.id);
        ...
        await tx.programStage.create({
          data: { programId: created.id, name: "Application", order: 1, isInitial: true },
        });
    ...
    return safeError(error, { route: "/api/programs", method: "POST" });
  }
}
